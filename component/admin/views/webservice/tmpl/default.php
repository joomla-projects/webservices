...
$action = JRoute::_('index.php?option=com_webservices&view=webservice');
...
<script type="text/javascript">
	jQuery(document).ready(function () {
		jQuery('body').on('click', '.fields-add-new-row', function(e){
			...
			{
				alert('<?php echo JText::_('COM_WEBSERVICES_WEBSERVICE_TASK_ADD_NEW_TASK_ERROR', true, true) ?>');
			}
			...
			var disabled = jQuery(this).val() == 0;
			var $operation = $parent.find('.ws-operation-configuration')
				.prop('disabled', disabled);
			$operation.find('.chzn-done').prop('disabled', disabled).trigger("liszt:updated");

			if (disabled)
			{
				$operation.find('.radio label').addClass('disabled').prop('disabled', disabled)
					.not('.active').hide();
			}
			else
			{
				$operation.find('.radio label').removeClass('disabled').prop('disabled', disabled)
					.not('.active').show();
			}
			...
			$editFields.find(':input').each(function(){
