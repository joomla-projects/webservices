 ...
 * @since       1.4
 */
class JApiSoapTransformBase implements JApiSoapTransformInterface
{
	/**
	...
	public function wsdlField($field, &$sequence, &$typeSchema, $elementName, $validateOptional = false, $extraFields = array())
	{
		...
		if (!isset($this->element['minOccurs']))
		{
			$this->element->addAttribute(
				'minOccurs',
				(($validateOptional && JApiHalHelper::isAttributeTrue($field, 'isRequiredField') || !$validateOptional) ? '1' : '0')
			);
		}

		if (!isset($this->element['maxOccurs']))
		{
			$this->element->addAttribute('maxOccurs', JApiHalHelper::attributeToString($field, 'maxOccurs', 1));
		}
