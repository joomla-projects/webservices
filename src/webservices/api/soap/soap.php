 ...
 * @since  1.2
 */
class JApiSoapSoap extends JApi
{
	/**
	 * Webservice object
	 *
	 * @var    JApiHalHal  Webservice
	 * @since  1.4
	 */
	...
	public function __construct($options = null)
	{
		...
		$this->webservice = new JApiHalHal($options);
	...
	public function apiSoap()
	{
		...
		$operation = new JApiSoapOperationOperation($this->webservice);
	...
	public function checkWSDL()
	{
		try
		{
			...
			$this->wsdlPath = JApiSoapHelper::getWebserviceFilePath(
		...
		$this->wsdl = JApiSoapHelper::generateWsdl($this->webservice->configuration, $this->wsdlPath);
	...
	public function render()
	{
		...
		else
		{
			...
			if ($this->webservice->statusCode >= 300)
			{
				...
				$body = JApiSoapHelper::createSoapFaultResponse($this->webservice->statusCode . ' ' . $this->webservice->statusText);
			}
			else
			{
				$body = $this->getBody();
		...
		JFactory::$document = new JApiSoapDocumentDocument($documentOptions, ($this->operation == 'wsdl' ? 'xml' : 'soap+xml'));
