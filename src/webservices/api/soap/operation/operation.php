 ...
 * @since       1.4
 */
class JApiSoapOperationOperation
{
	/**
	 * Webservice object
	 *
	 ...
	 * Constructor.
	 *
	 * @param   JApiHalHal  $webservice  Webservice object
	...
	public function readList($data)
	{
		...
		$filters = JApiHalHelper::getFilterFields($this->webservice->configuration->operations->read->list, true);
		...
		$outputResources = JApiSoapHelper::getOutputResources($this->webservice->configuration->operations->read->list, 'listItem', true);

		if ($arr['_embedded'] && $arr['_embedded']['item'])
		{
			$response = JApiSoapHelper::selectListResources($outputResources, $arr['_embedded']['item']);
		}
		else
		{
			$response = array();
	...
	public function readItem($data)
	{
		...
		$this->setOperation('read');
		$dataGet = $this->webservice->options->get('dataGet', array());
		$primaryKeysFromFields = JApiHalHelper::getFieldsArray($this->webservice->configuration->operations->read->item, true);
		...
		$arr = $this->webservice->hal->toArray();
		$outputResources = JApiSoapHelper::getOutputResources($this->webservice->configuration->operations->read->item, '', true);

		$response = JApiSoapHelper::selectListResources($outputResources, array($arr));
