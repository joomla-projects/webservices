...
class JApiSoapWsdl
{
	...
	public function addOperation(&$wsdl, $name, $inputFields, $outputFields, $validateOptionalInput = false, $validateOptionalOutput = false)
	{
		...
		JApiSoapHelper::addElementFields($inputFields, $this->typeSchema, '', $validateOptionalInput, $name);
		JApiSoapHelper::addElementFields($outputFields, $this->typeSchema, '', $validateOptionalOutput, $name . 'Response');
	}
	...
	public function generateWsdl($wsdlPath)
	{
		...
		if (isset($this->webserviceXml->operations))
		{
						...
						'name' => 'list', 'transform' => 'arrayrequired', 'fields' =>
							array(
								array(
									...
									'fields' => JApiSoapHelper::getOutputResources($this->webserviceXml->operations->read->list, 'listItem')
									)
								)
							)
						);
			...
			if (isset($this->webserviceXml->operations->read->item))
			{
				...
				$outputFields = array(
					array(
						...
						'typeName' => 'item',
						'transform' => 'arrayrequired',
						'fields' => JApiSoapHelper::getOutputResources($this->webserviceXml->operations->read->item)
						)
					);
			...
			if (isset($this->webserviceXml->operations->create))
			{
				...
				$outputFields = array(JApiSoapHelper::getResultResource($this->webserviceXml->operations->create));
			...
			if (isset($this->webserviceXml->operations->update))
			{
				...
				$outputFields = array(JApiSoapHelper::getResultResource($this->webserviceXml->operations->update));
			...
			if (isset($this->webserviceXml->operations->delete))
			{
				...
				$outputFields = array(JApiSoapHelper::getResultResource($this->webserviceXml->operations->delete));
			...
			if (isset($this->webserviceXml->operations->task))
			{
				foreach ($this->webserviceXml->operations->task->children() as $taskName => $task)
				{
					...
					$outputFields = array(JApiSoapHelper::getResultResource($task));
