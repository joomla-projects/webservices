...
class JApiHalHal extends JApi
{
	...
	public function __construct($options = null)
	{
		...
		$this->hal = new JApiHalDocumentResource('');

		if (!empty($this->webserviceName))
		{
			if (empty($this->webserviceVersion))
			{
				$this->webserviceVersion = JApiHalHelper::getNewestWebserviceVersion($this->client, $this->webserviceName);
			}

			$this->webservice = JApiHalHelper::getInstalledWebservice($this->client, $this->webserviceName, $this->webserviceVersion);
			...
			$this->configuration = JApiHalHelper::loadWebserviceConfiguration(
		...
		// OAuth2 check
		if (JBootstrap::getConfig('webservices_permission_check', 1) == 0)
		{
			$this->permissionCheck = 'scope';
		}
		elseif (JBootstrap::getConfig('webservices_permission_check', 1) == 1)
		{
	...
	public function execute()
	{
		...
		try
		{
			...
			else
			{
				...
				if (JBootstrap::getConfig('webservices_default_page_authorization', 0) == 1 && !$this->loginUser(''))
				{
					return false;
				}
		...
		if (!empty($messages))
		{
			...
			if (JBootstrap::getConfig('debug_webservices', 0) == 0)
			{
	...
	public function apiDefaultPage()
	{
		...
		$documentationCurieAdmin = new JApiHalDocumentLink('/index.php?option={rel}&amp;format=doc&amp;webserviceClient=administrator',
			'curies', 'Documentation Admin', 'Admin', null, true
		);
		$documentationCurieSite = new JApiHalDocumentLink('/index.php?option={rel}&amp;format=doc&amp;webserviceClient=site',
		...
		$this->hal->setLink(new JApiHalDocumentLink(JUri::base(), 'base', JText::_('LIB_WEBSERVICES_API_HAL_WEBSERVICE_DOCUMENTATION_DEFAULT_PAGE')));

		$webservices = JApiHalHelper::getInstalledWebservices();
	...
	public function apiRead()
	{
		...
		{
			$functionName = JApiHalHelper::attributeToString($currentConfiguration, 'functionName', 'getItems');
		...
		// Getting single item
		$functionName = JApiHalHelper::attributeToString($currentConfiguration, 'functionName', 'getItem');
	...
	public function apiCreate()
	{
		...
		$functionName = JApiHalHelper::attributeToString($this->operationConfiguration, 'functionName', 'save');
	...
	public function apiDelete()
	{
		...
		$model = $this->triggerFunction('loadModel', $this->elementName, $this->operationConfiguration);
		$functionName = JApiHalHelper::attributeToString($this->operationConfiguration, 'functionName', 'delete');
		$data = $this->triggerFunction('processPostData', $this->options->get('data', array()), $this->operationConfiguration);
		...
		// Prepare parameters for the function
		if (strtolower(JApiHalHelper::attributeToString($this->operationConfiguration, 'dataMode', 'model')) == 'table')
		{
			$primaryKeys = array();
	...
	public function apiTask()
	{
		...
		if (!empty($task))
		{
			...
			$functionName = JApiHalHelper::attributeToString($taskConfiguration, 'functionName', $task);
	...
	public function setForRenderList($items, $configuration)
	{
		...
		if (!empty($items))
		{
			...
			foreach ($items as $itemValue)
			{
				...
				$embedItem = new JApiHalDocumentResource('item', $item);
	 ...
	 *
	 * @param   JApiHalDocumentResource  $resourceDocument  Resource document for binding the resource
	 ...
	 *
	 * @return JApiHalDocumentResource
	 */
					...
					if ($resource['displayGroup'] == '_links')
					{
						...
						$resourceDocument->setLink(
							new JApiHalDocumentLink(
								...
								$resource['hrefLang'],
								JApiHalHelper::isAttributeTrue($resource, 'linkTemplated')
	...
	public function loadResourceFromConfiguration($configuration)
	{
		if (isset($configuration->resources->resource))
		{
			...
			{
				$resource = JApiHalHelper::getXMLElementAttributes($resourceXML);
	...
	public function render()
	{
		...
		if (!empty($token))
		{
			$this->setUriParams(JBootstrap::getConfig('oauth2_token_param_name', 'access_token'), $token);
		}
		...
		else
		{
			...
			JFactory::$document = new JApiHalDocumentDocument($documentOptions);
	 ...
	 *
	 * @return  JApiHalHal      An instance of itself for chaining
	 */
	...
	public function processPostData($data, $configuration)
	{
		...
		if (!empty($data) && !empty($configuration->fields))
		{
			...
			{
				$fieldAttributes = JApiHalHelper::getXMLElementAttributes($field);
			...
			if (JApiHalHelper::isAttributeTrue($configuration, 'strictFields'))
			{
				$data = $dataFields;
			}
		}
	...
	public function validatePostData($model, $data, $configuration)
	{
		...
		$validateMethod = strtolower(JApiHalHelper::attributeToString($configuration, 'validateData', 'none'));
		...
		if ($validateMethod == 'form')
		{
			...
			$app->enqueueMessage(JText::_('LIB_WEBSERVICES_API_HAL_WEBSERVICE_FUNCTION_DONT_EXIST'), 'error');
		...
		{
			$validateMethod = strtolower(JApiHalHelper::attributeToString($configuration, 'validateDataFunction', 'validate'));
	...
	public function checkRequiredFields($data, $configuration)
	{
		if (!empty($configuration->fields))
		{
			foreach ($configuration->fields->field as $field)
			{
				if (JApiHalHelper::isAttributeTrue($field, 'isRequiredField'))
				{
	...
	public function isOperationAllowed()
	{
		...
		if (!JApiHalHelper::isPublishedWebservice($this->client, $this->webserviceName, $this->webserviceVersion) && !empty($this->webserviceName))
		{
		...
		$authorized = false;
		JFactory::getApplication()->triggerEvent('JApiHalPermissionCheck',
	...
	public function getHelperObject()
	{
		...
		$helperFile = JApiHalHelper::getWebserviceFile($this->client, strtolower($this->webserviceName), $version, 'php', $this->webservicePath);
	...
	public function getDynamicModelObject($configuration)
	{
		...
		$tableName = JApiHalHelper::attributeToString($configuration, 'tableName', '');
		...
		$paginationPrefix = '';
		$filterFields = JApiHalHelper::getFilterFields($configuration);
	...
	public function getPrimaryFields($configuration)
	{
		...
		if (!empty($configuration->fields))
		{
			foreach ($configuration->fields->field as $field)
			{
				$isPrimaryField = JApiHalHelper::isAttributeTrue($field, 'isPrimaryField');
	...
	public function loadModel($elementName, $configuration)
	{
		...
		$isAdmin = JApiHalHelper::isAttributeTrue($configuration, 'isAdminClass');
		...
		$dataMode = strtolower(JApiHalHelper::attributeToString($configuration, 'dataMode', 'model'));
	...
	public function assignValueToResource($resource, $value, $attribute = 'fieldFormat')
	{
		...
		$transform = JApiHalHelper::attributeToString($resource, 'transform', '');
	...
	public function transformField($fieldType, $definition, $directionExternal = true)
	{
		...
		if ($className instanceof JApiHalTransformInterface)
		{
	...
	public function triggerFunction($functionName)
	{
		...
		$result = JFactory::getApplication()->triggerEvent('JApiHalBefore' . $functionName, $temp);
		...
		JFactory::getApplication()->triggerEvent('JApiHalAfter' . $functionName, $temp);
	...
	public function apiFillPrimaryKeys(&$primaryKeys, $configuration = null)
	{
		...
		{
			$primaryKeysFromFields = JApiHalHelper::getFieldsArray($configuration, true);
